const sv = {
  nav: {
    brand: 'Prepper',
    dashboard: 'Översikt',
    products: 'Mat',
    water: 'Vatten',
  },
  common: {
    loading: 'Laddar…',
    save: 'Spara',
    saving: 'Sparar…',
    cancel: 'Avbryt',
    delete: 'Ta bort',
    edit: 'Redigera',
    confirm: 'Bekräfta',
    undo: 'Ångra',
  },
  categories: {
    WATER: 'Vatten',
    PRESERVED_FOOD: 'Konserver',
    DRY_GOODS: 'Torrvaror',
    FREEZE_DRIED: 'Frystorkat',
    MEDICINE: 'Medicin',
    FUEL: 'Bränsle',
    OTHER: 'Övrigt',
  },
  units: {
    LITERS: 'L',
    KG: 'kg',
    CANS: 'burkar',
    PIECES: 'st',
    GRAMS: 'g',
  },
  dashboard: {
    title: 'Översikt',
    all_good: 'Allt bra!',
    all_good_desc: 'Inga utgångna, snart utgångna eller låglagerposter.',
    empty: 'Lagret är tomt',
    empty_desc: 'Lägg till mat eller vatten för att börja spåra ditt lager.',
    expired_heading: 'Utgångna ({{count}})',
    expiring_heading: 'Snart utgångna ({{count}})',
    low_stock_heading: 'Lågt lager ({{count}})',
    product_fallback: 'Produkt #{{id}}',
    expires: 'Bäst före {{date}}',
    stats_products: 'Produkter',
    stats_at_target: '{{count}} på mål',
    stats_readiness: 'Beredskap',
    stats_at_risk: 'I riskzon',
    stats_n_expired: '{{count}} utgångna',
    stats_n_expiring: '{{count}} snart utgångna',
    stats_n_low: '{{count}} låga',
    stats_closest_expiry: 'Närmast datum',
    stats_expires_today: 'Idag',
    stats_one_day: '1 dag',
    stats_days_remaining: '{{count}} dagar',
    stats_expiry_beyond_30d: '>30d',
    category_breakdown: 'Per kategori',
    category_at_target: '{{count}}/{{total}} på mål',
  },
  products: {
    page_title: 'Mat',
    add_button: '+ Lägg till mat',
    loading_error: 'Kunde inte ladda mat.',
    empty: 'Ingen mat lagrad ännu.',
    not_found: 'Mat hittades inte.',
    back: '← Mat',
    back_from_detail: '← Tillbaka till mat',
    target: 'mål {{qty}} {{unit}}',
    stock_batches_title: 'Lager',
    add_stock_button: '+ Lägg till lager',
    loading_stock: 'Laddar lager…',
    no_stock: 'Tomt...',
    delete_title: 'Ta bort',
    delete_water_title: 'Ta bort',
    delete_confirm:
      'Detta tar permanent bort "{{name}}" och alla {{count}} lagerpartier. Det går inte att ångra.',
    ok_badge: 'OK',
    low_badge: 'Låg',
  },
  product_form: {
    add_title: 'Lägg till mat',
    edit_title: 'Redigera mat',
    edit_water_title: 'Redigera vatten',
    back_to_product: '← Tillbaka till mat',
    back_to_products: '← Tillbaka till mat',
    name_label: 'Namn',
    name_placeholder: 'ex. Mineralvatten',
    category_label: 'Kategori',
    unit_label: 'Enhet',
    target_qty_label: 'Målkvantitet',
    target_qty_liters_label: 'Målkvantitet (Liter)',
    target_qty_placeholder: 'ex. 100',
    persons_label: 'Antal personer',
    persons_hint: '{{days}} dagar × {{lpd}} L/person/dag',
    notes_label: 'Anteckningar (valfritt)',
    notes_placeholder: 'Extra information…',
  },
  stock_form: {
    modal_title: 'Lägg till lagerparti',
    quantity_label: 'Kvantitet ({{unit}})',
    quantity_placeholder: 'ex. 40',
    sub_type_label: 'Variant (valfritt)',
    sub_type_placeholder: 'ex. Penne pasta',
    purchased_date_label: 'Inköpsdatum',
    expiry_date_label: 'Utgångsdatum',
    location_label: 'Plats (valfritt)',
    location_placeholder: 'ex. Källarhylla A',
    notes_label: 'Anteckningar (valfritt)',
    add_button: 'Lägg till lager',
    adding_button: 'Lägger till…',
    optional_details: 'Valfria uppgifter',
    name_label: 'Namn',
    name_placeholder: 'ex. Blå flaska hyl B',
  },
  stock_entry: {
    consume_next: 'Kortast datum',
    quantity_label: 'Kvantitet',
    name_label: 'Namn',
    sub_type_label: 'Variant',
    location_label: 'Plats',
    purchased_label: 'Tillagd',
    expires_label: 'Bäst före',
    use_one_button: '− 1',
    update_qty_button: 'Ändra antal',
    remove_batch_button: 'Ta bort parti',
    delete_confirm: 'Ta bort den här partiet från lagret?',
    removed: 'Parti borttaget',
  },
  category_filter: {
    all: 'Alla',
  },
  water_widget: {
    title: 'Vattenförsörjning',
    setup_prompt: 'Ställ in din dricksvattenreserv för att säkert spåra din mest kritiska resurs.',
    setup_button: 'Börja spåra',
    default_name: 'Dricksvatten',
    manage_link: 'Hantera partier',
    add_stock: 'Lägg till',
    stored: 'lagrat',
    target: 'Mål',
  },
  action: {
    WATER: {
      APPROACHING: 'Rotera till daglig användning och fyll på med nytt förråd',
      EXPIRED: 'Kassera och ersätt omedelbart — drick inte',
    },
    PRESERVED_FOOD: {
      APPROACHING: 'Flytta till vardagsköket och köp nytt till förrådet',
      EXPIRED: 'Kontrollera förpackningen för skador, svullnad eller lukt innan kassering',
    },
    DRY_GOODS: {
      APPROACHING: 'Flytta till matlagningsrotation och ersätt i förrådet',
      EXPIRED: 'Kontrollera för fukt, skadedjur eller dålig lukt innan kassering',
    },
    FREEZE_DRIED: {
      APPROACHING: 'Inkludera i vanliga måltider för rotation — beställ ersättning',
      EXPIRED: 'Kontrollera förpackningens integritet; om förseglad och luktfri kan den fortfarande vara användbar',
    },
    MEDICINE: {
      APPROACHING: 'Rådfråga apotekare om ersättning; låt inte detta löpa ut',
      EXPIRED: 'Lämna in på apotek för destruktion — använd inte',
    },
    FUEL: {
      APPROACHING: 'Använd i ett utrustnings- eller generatortest; fyll på med nytt förråd',
      EXPIRED: 'Lämna till miljöstation för farligt avfall — använd inte i utrustning',
    },
    OTHER: {
      APPROACHING: 'Bedöm skicket och planera för rotation',
      EXPIRED: 'Bedöm skicket och ersätt vid behov',
    },
  },
  not_found: {
    message: 'Sidan hittades inte.',
    cta: 'Gå till översikten →',
  },
  errors: {
    not_found: 'Hittades inte',
    client_error: 'Klientfel — kontrollera dina uppgifter',
    something_went_wrong: 'Något gick fel',
  },
  lang_label: {
    sv: 'SV',
    en: 'EN',
  },
  offline: {
    title: 'Ingen serveranslutning',
    message: 'Kontrollerar anslutning…',
    indicator: 'Offline',
  },
} as const

export default sv
