const en = {
  nav: {
    brand: 'Prepper',
    dashboard: 'Dashboard',
    products: 'Food',
    water: 'Water',
  },
  common: {
    loading: 'Loading…',
    save: 'Save',
    saving: 'Saving…',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    confirm: 'Confirm',
    undo: 'Undo',
  },
  categories: {
    WATER: 'Water',
    PRESERVED_FOOD: 'Preserved Food',
    DRY_GOODS: 'Dry Goods',
    FREEZE_DRIED: 'Freeze-Dried',
    MEDICINE: 'Medicine',
    FUEL: 'Fuel',
    OTHER: 'Other',
  },
  units: {
    LITERS: 'L',
    KG: 'kg',
    CANS: 'cans',
    PIECES: 'pcs',
    GRAMS: 'g',
  },
  dashboard: {
    title: 'Dashboard',
    all_good: 'All good!',
    all_good_desc: 'No expired, expiring, or low-stock items.',
    empty: 'Storage is empty',
    empty_desc: 'Add food or water to start tracking your prepper stock.',
    expired_heading: 'Expired ({{count}})',
    expiring_heading: 'Expiring soon ({{count}})',
    low_stock_heading: 'Low stock ({{count}})',
    product_fallback: 'Product #{{id}}',
    expires: 'Expires {{date}}',
    stats_products: 'Products',
    stats_at_target: '{{count}} at target',
    stats_readiness: 'Readiness',
    stats_at_risk: 'At risk',
    stats_n_expired: '{{count}} expired',
    stats_n_expiring: '{{count}} expiring',
    stats_n_low: '{{count}} low',
    stats_closest_expiry: 'Closest expiry',
    stats_expires_today: 'Today',
    stats_one_day: '1 day',
    stats_days_remaining: '{{count}} days',
    stats_expiry_beyond_30d: '>30d',
    category_breakdown: 'By category',
    category_at_target: '{{count}}/{{total}} at target',
  },
  products: {
    page_title: 'Food',
    add_button: '+ Add food',
    loading_error: 'Failed to load food.',
    empty: 'No food yet.',
    not_found: 'Food not found.',
    back: '← Food',
    back_from_detail: '← Back to food',
    target: 'target {{qty}} {{unit}}',
    stock_batches_title: 'Stock batches',
    add_stock_button: '+ Add stock',
    loading_stock: 'Loading stock…',
    no_stock: 'No stock entries yet.',
    delete_title: 'Delete food',
    delete_water_title: 'Delete water',
    delete_confirm:
      'This will permanently delete "{{name}}" and all {{count}} stock batches. This cannot be undone.',
    ok_badge: 'OK',
    low_badge: 'Low',
  },
  product_form: {
    add_title: 'Add food',
    edit_title: 'Edit food',
    edit_water_title: 'Edit water',
    back_to_product: '← Back to food',
    back_to_products: '← Back to food',
    name_label: 'Name',
    name_placeholder: 'e.g. Mineral Water',
    category_label: 'Category',
    unit_label: 'Unit',
    target_qty_label: 'Target quantity',
    target_qty_liters_label: 'Target quantity (Liters)',
    target_qty_placeholder: 'e.g. 100',
    persons_label: 'Number of persons',
    persons_hint: '{{days}} days × {{lpd}} L/person/day',
    notes_label: 'Notes (optional)',
    notes_placeholder: 'Any extra info…',
  },
  stock_form: {
    modal_title: 'Add stock batch',
    quantity_label: 'Quantity ({{unit}})',
    quantity_placeholder: 'e.g. 40',
    sub_type_label: 'Variant (optional)',
    sub_type_placeholder: 'e.g. Penne pasta',
    purchased_date_label: 'Purchase date',
    expiry_date_label: 'Expiry date',
    location_label: 'Location (optional)',
    location_placeholder: 'e.g. Basement shelf A',
    notes_label: 'Notes (optional)',
    add_button: 'Add stock',
    adding_button: 'Adding…',
    optional_details: 'Optional details',
    name_label: 'Name',
    name_placeholder: 'e.g. Blue bottle shelf B',
  },
  stock_entry: {
    consume_next: 'Consume next',
    quantity_label: 'Quantity',
    name_label: 'Name',
    sub_type_label: 'Variant',
    location_label: 'Location',
    purchased_label: 'Purchased',
    expires_label: 'Expires',
    use_one_button: '− 1',
    update_qty_button: 'Edit qty',
    remove_batch_button: 'Remove batch',
    delete_confirm: 'Remove this batch from storage?',
    removed: 'Batch removed',
  },
  category_filter: {
    all: 'All',
  },
  water_widget: {
    title: 'Water Supply',
    setup_prompt: 'Set up your drinking water reserve to track your most critical resource securely.',
    setup_button: 'Start tracking',
    default_name: 'Drinking Water',
    manage_link: 'Manage batches',
    add_stock: 'Add stock',
    stored: 'stored',
    target: 'Target',
  },
  action: {
    WATER: {
      APPROACHING: 'Rotate to daily use and refill with fresh stock',
      EXPIRED: 'Discard and replace immediately — do not consume',
    },
    PRESERVED_FOOD: {
      APPROACHING: 'Move to everyday pantry rotation and repurchase for the stash',
      EXPIRED: 'Inspect packaging for damage, swelling, or odor before discarding',
    },
    DRY_GOODS: {
      APPROACHING: 'Move to everyday cooking rotation and replace in stash',
      EXPIRED: 'Inspect for moisture, pests, or off-odor before discarding',
    },
    FREEZE_DRIED: {
      APPROACHING: 'Include in regular meals to rotate — reorder replacement',
      EXPIRED: 'Check packaging integrity; if sealed and odor-free may still be usable',
    },
    MEDICINE: {
      APPROACHING: 'Consult a pharmacist about replacement; do not let this lapse',
      EXPIRED: 'Dispose via pharmacy take-back program — do not use',
    },
    FUEL: {
      APPROACHING: 'Use in an equipment or generator test run; refill with fresh stock',
      EXPIRED: 'Dispose at a hazardous waste facility — do not use in equipment',
    },
    OTHER: {
      APPROACHING: 'Review item condition and plan for rotation',
      EXPIRED: 'Assess condition and replace if necessary',
    },
  },
  not_found: {
    message: 'Page not found.',
    cta: 'Go to dashboard →',
  },
  errors: {
    not_found: 'Not found',
    client_error: 'Client error — please check your input',
    something_went_wrong: 'Something went wrong',
  },
  lang_label: {
    sv: 'SV',
    en: 'EN',
  },
  offline: {
    title: 'Backend unreachable',
    message: 'Checking connection…',
    indicator: 'Offline',
  },
} as const

export default en
